...
from pdf2data.support import (convert_pdfminersix, get_doc_list,
                              get_string_from_box, remove_pdf_images,
                              verify_string_in_list)
...
class TextFileGenerator(BaseModel):
    ...
    def pdf_to_cermxml(self) -> None:
        ...
        for document in converted_doc_list:
            shutil.move(
                self.input_folder + "/" + document, self.output_folder + "/" + document
            )
    ...
    def pdf_to_muhtml(self) -> None:
        ...
        for doc in initial_doc_list:
            ...
            shutil.move(
                document_name + ".html",
                self.output_folder + "/" + document_name + ".html",
            )
    ...
    def pdf_to_miner(self, file_format: str) -> None:
        ...
        if file_format not in valid_formats:
            raise AttributeError(
                f"the file format is not valid. Only file format in this list are valid:{valid_formats}"
            )
        initial_doc_list: List[str] = get_doc_list(self.input_folder, ".pdf")
        for doc in initial_doc_list:
            document_name: str = os.path.splitext(doc)[0]
            if file_format == "txt":
                ...
                text = convert_pdfminersix(
                    self.input_folder + "/" + doc, format=file_format
                )
...
class TextExtractor(BaseModel):
    ...
    def extract_txt(
        self, file_name: str, strings_to_remove: List[str] = []
    ) -> Dict[str, List[str]]:
    ...
    def extract_layoutparser(
        self, file_name: str, layout: Dict[str, Any], strings_to_remove: List[str] = []
    ) -> Dict[str, List[Any]]:
