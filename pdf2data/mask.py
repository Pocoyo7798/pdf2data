...
from pdf2data.support import iou, order_horizontal, block_organizer
...
class LayoutParser(BaseModel):
    ...
    _existing_models: List[str] = PrivateAttr(default=set(["PrimaLayout_mask_rcnn_R_50_FPN_3x", "PubLayNet_faster_rcnn_R_50_FPN_3x", "PubLayNet_mask_rcnn_X_101_32x8d_FPN_3x", "TableBank_faster_rcnn_R_50_FPN_3x", "TableBank_faster_rcnn_R_101_FPN_3x", "microsoft/table-transformer-detection"]))
    
    def model_post_init(self, __context: Any) -> None:
        if self.model not in self._existing_models:
            raise AttributeError(f"The specified model is not available, the available, models are {self._existing_models}")
        if self.table_model not in self._existing_models and self.table_model is not None:
            raise AttributeError(f"The specified table model is not available, the available, models are {self._existing_models}")
        try:
        ...
        if layout_parser is False:
            ...
            extra_config=["MODEL.ROI_HEADS.SCORE_THRESH_TEST", self.model_threshold],
            label_map=labels,
            )
        else:
            ...
            extra_config=["MODEL.ROI_HEADS.SCORE_THRESH_TEST", self.model_threshold] # Optional
        )
        if self.table_model is not None:
            try:
                ...
                layout_parser: bool = False
            if layout_parser is False:
                self._table_model = TableTransformerForObjectDetection.from_pretrained(self.table_model)
            elif self.table_model_path is not None:
                self._model = lp.Detectron2LayoutModel(
                f"{self.table_model_path}/config.yaml",
                f"{self.table_model_path}/model_final.pth",
                extra_config=["MODEL.ROI_HEADS.SCORE_THRESH_TEST", self.table_model_threshold],
                label_map=labels,
                )
            else:
    ...
    @staticmethod
    def generate_layout_lp(model: Any, page: Any, width: float, pdf_width: float, height:float, pdf_height:float) -> Dict[str, Any]:
        ...
        for entry in layout:
            ...
            if entry.type in TEXT_WORDS_REGISTRY:
                ...
                types.append("Figure")
                exist_figure = True
        return {"boxes": boxes, "scores": scores, "types": types, "table_boxes": table_boxes, "table_scores": table_scores, "table_type": table_types, "exist_figure": exist_figure}
    
    @staticmethod
    def generate_layout_hf(model: Any, page: Any, width: float, pdf_width: float, height:float, pdf_height:float, threshold: str) -> Dict[str, Any]:
        ...
        return {"boxes": [], "scores": [], "types": [], "table_boxes": boxes, "table_scores": table_scores, "table_type": types, "exist_figure": False}
    ...
    def get_layout(self, pdf_path: str, iou_max: float):
        ...
        for page in page_list:
            ...
            if self.model == "microsoft/table-transformer-detection":
                ...
                first_layout = LayoutParser.generate_layout_lp(self._model, page, width, height, pdf_height, pdf_width)
            ...
            if self.table_model is not None:
                if self.table_model == "microsoft/table-transformer-detection":
                    sec_layout: Dict[str, Any] = LayoutParser.generate_layout_hf(self._table_model, page, width, height, pdf_height, pdf_width, self.model_threshold)
                else:
                    sec_layout = LayoutParser.generate_layout_lp(self._table_model, page, width, height, pdf_height, pdf_width)
...
LAYOUT_PARSER_LABELS_REGISTRY: Dict[str, Dict[int, str]] = {"PrimaLayout_mask_rcnn_R_50_FPN_3x": {1:"TextRegion", 2:"ImageRegion", 3:"TableRegion", 4:"MathsRegion", 5:"SeparatorRegion", 6:"OtherRegion"},
                                                     "PubLayNet_faster_rcnn_R_50_FPN_3x": {0: "Text", 1: "Title", 2: "List", 3:"Table", 4:"Figure"},
                                                     "PubLayNet_mask_rcnn_X_101_32x8d_FPN_3x": {0: "Text", 1: "Title", 2: "List", 3:"Table", 4:"Figure"},
                                                     "TableBank_faster_rcnn_R_50_FPN_3x": {0: "Table"},
                                                     "TableBank_faster_rcnn_R_101_FPN_3x": {0: "Table"}
                                                     }

LAYOUT_PARSER_CONFIG_REGISTRY: Dict[str, str] = {"PrimaLayout_mask_rcnn_R_50_FPN_3x": "lp://PrimaLayout/mask_rcnn_R_50_FPN_3x/config",
